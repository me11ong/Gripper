...
class CostMatrix(nn.Module):
    ...
    def forward(self, x_img, x_text, depth):
        ...
        for text in x_text:
            text = list(set(text))
            text_feature = self.get_text_embeds(text, self.templates, self.clip_model)  # [1, 1, 512]
            text_features_global.append(text_feature.squeeze(0))  # [1, 512]
        ...
        text_features_spatial = text_features_spatial.view(-1, 576, 512)  # [B, 576, 512]

        visual_features_norm = F.normalize(visual_features, dim=-1)  # [B, 576, 512]
        text_features_norm = F.normalize(text_features_spatial, dim=-1)  # [B, 576, 512]
...
class TextGuidedFusion(nn.Module):
    ...
    def forward(self, spatial_features, text_embedding):
        """
        Args:
            spatial_features: [B, C, H, W] - FPN feature map
            text_embedding: [B, text_dim] - Global text embedding

        Returns:
            modulated_features: [B, C, H, W] - Text-modulated features
        """
        B, C, H, W = spatial_features.shape

        modulation_params = self.modulation(text_embedding)  # [B, C*2]

        gamma, beta = modulation_params.chunk(2, dim=-1)  # [B, C], [B, C]
...
class QuadMRConv4d(nn.Module):
    ...
    def roll_and_connect(self, x, x_rolls):
        ...
        weight_sum = torch.zeros(B, 1, H, W, device=x.device)
        ...
        all_dists_cat = torch.cat(all_dists, dim=1)  # [B, R, H, W]
...
if has_mmdet:
    ...
    def greedyvig_s_costmatrix(pretrained=False, **kwargs):
        model = GreedyViG_CostMatrix(blocks=[[4,4], [4,4], [12,4], [3,3]],
...
try:
    ...
    class TextGuidedFPN(FPN):
        ...
        def set_text_embeddings(self, text_embeddings):
            """
            Set text embeddings before forward pass

            Args:
                text_embeddings: [B, text_dim] - Global text embeddings from CLIP
            """
        ...
        def forward(self, inputs):
            ...
            Returns:
                tuple: FPN feature maps [P2, P3, P4, P5, P6] with text fusion
            """
            # Standard FPN forward
            fpn_outputs = super(TextGuidedFPN, self).forward(inputs)

            # Apply text-guided fusion if text embeddings are available
            if self.text_embeddings is not None:
                fused_outputs = []
                for i, feat in enumerate(fpn_outputs):
                    # Apply text fusion to each FPN level
                    fused_feat = self.text_fusions[i](feat, self.text_embeddings)
                    fused_outputs.append(fused_feat)
                return tuple(fused_outputs)
            else:
                # No text available, return original FPN outputs
                return fpn_outputs
